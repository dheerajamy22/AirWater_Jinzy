...
class Document_Requested_Request extends StatefulWidget {
  const Document_Requested_Request({Key? key}) : super(key: key);

  @override
  _Document_Requested_RequestState createState() =>
      _Document_Requested_RequestState();
}

class _Document_Requested_RequestState
    extends State<Document_Requested_Request> {
  ...
  Future<List<DocumentModel>> getAllDocData() async {
    ...
    for (var data in jsonArray) {
      ...
      for (int i = 0; i < doc_.length; i++) {
        if (doc_[i].rfdoc_status == 'Submited') {
