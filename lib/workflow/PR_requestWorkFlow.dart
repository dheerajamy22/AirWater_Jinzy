...
class _PurchaseRequestWorkflowState extends State<PurchaseRequestWorkflow> {
  List<PurchaseRequestWorkFlowModel> workflow_list = [];
  ...
  void getPrWorkFlowList(String status) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonObject['status'] == "1") {
        ...
        List<PurchaseRequestWorkFlowModel> tempList = [];

        for (var array_data in jsonarray) {
          PurchaseRequestWorkFlowModel prItem = PurchaseRequestWorkFlowModel(
          ...
          // Apply filter
          if (status == 'All' ||
              (status == 'Approved' && prItem.status == 'Approved') ||
              (status == 'Declined' && prItem.status == 'Rejected') ||
              (status == 'In Review' && prItem.status == 'Pending')) {
            tempList.add(prItem);
          }
        }
